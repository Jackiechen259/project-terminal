...
[下载](#下载) · [功能特性](#功能特性) · [快速开始](#快速开始) · [架构设计](#架构设计) · [安全特性](#安全特性) · [发布说明](./CHANGELOG.md)
...
- **终端分屏 (Split Panes)** — 支持在工作区内最多拆分 4 个终端窗格（左右或上下拆分），支持快捷键与拖拽拆分。
- **经身份验证的远程访问网关** — 内置安全 HTTP/WebSocket 网关，支持通过浏览器或移动端远程查看与控制终端会话。
...
对于基于密钥的身份验证，建议配合使用 `ssh-agent`。Project Terminal 仅保存私钥**路径**，绝不保存私钥内容。密码验证可选择将密码保存到 Windows 凭据管理器；密码绝不会写入 `ssh-connections.json`。
...
- 终端输出历史不会持久化存储到磁盘。
- 绝不会自动接受未知的 SSH 主机密钥。
- 暂未内置代码编辑器、Git GUI 或端口转发图形界面。文件侧边栏仅浏览项目目录，并非完整的系统文件管理器。
