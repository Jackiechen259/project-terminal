...
#[tauri::command]
pub async fn get_general_settings(state: State<'_, AppState>) -> AppResult<Option<Value>> {
    let state = state.inner().clone();
    tauri::async_runtime::spawn_blocking(move || state.settings.get_value(GENERAL_SETTINGS_KEY))
        .await
        .map_err(|error| AppError::Configuration(format!("Settings worker failed: {error}")))?
}

#[tauri::command]
pub async fn save_general_settings(state: State<'_, AppState>, value: Value) -> AppResult<()> {
    let state = state.inner().clone();
...
#[tauri::command]
pub async fn load_collections(state: State<'_, AppState>) -> AppResult<CollectionSnapshot> {
    let state = state.inner().clone();
    tauri::async_runtime::spawn_blocking(move || state.collections.load())
        .await
        .map_err(|error| AppError::Configuration(format!("Collections worker failed: {error}")))?
}
...
#[tauri::command]
pub async fn list_project_memos(
    state: State<'_, AppState>,
    project_id: String,
    ...
    .await
    .map_err(|error| AppError::Configuration(format!("Memo worker failed: {error}")))?
}

#[tauri::command]
pub async fn save_project_memos(
    state: State<'_, AppState>,
    project_id: String,
...
#[tauri::command]
pub async fn delete_project_memo(state: State<'_, AppState>, memo_id: String) -> AppResult<()> {
    let state = state.inner().clone();
    tauri::async_runtime::spawn_blocking(move || {
...
pub async fn load_workspace_state(
    ...
    let state = state.inner().clone();
    tauri::async_runtime::spawn_blocking(move || state.workspace_state.load(&workspace_id))
        .await
        .map_err(|error| AppError::Configuration(format!("Workspace worker failed: {error}")))?
}
...
#[tauri::command]
pub async fn migrate_frontend_persistence(
    state: State<'_, AppState>,
            ...
            state.db.transaction(|transaction| {
                ...
                if let Some(settings) = payload.general_settings.as_ref() {
                    ...
                    let existing: Option<String> = transaction
                        ...
                        .optional()
                        .map_err(|error| {
                            database::error::sqlite("check general settings", error)
                        })?;
                    if existing.is_none() {
                        SettingsRepository::set_value_tx(
                            transaction,
                            GENERAL_SETTINGS_KEY,
                            settings,
                        )?;
                ...
                for (workspace_id, workspace_state) in &payload.workspace_states {
                    ...
                    if exists == 0 {
                        WorkspaceStateRepository::save_tx(
                            transaction,
                            workspace_id,
                            workspace_state,
                        )?;
    ...
    .await
    .map_err(|error| {
        AppError::Configuration(format!("Persistence-migration worker failed: {error}"))
    })?
}
