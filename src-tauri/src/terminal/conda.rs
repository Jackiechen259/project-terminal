...
/// Look for Conda executables in common Windows locations.
/// Prioritizes existing CONDA_EXE, then PATH, then standard installs.
pub fn detect_conda_installations() -> Vec<String> {
    ...
    if let Some(up) = std::env::var_os("USERPROFILE") {
        ...
        roots.push(p.join("anaconda3"));
        roots.push(p.join("miniconda3"));
        roots.push(p.join("miniforge3"));
    }
    ...
    for root in roots {
        let exe = root.join("Scripts").join("conda.exe");
        if exe.is_file() {
        ...
        if bat.is_file() {
            let s = bat.to_string_lossy().into_owned();
            if !found.contains(&s) {
                found.push(s);
            }
        }
